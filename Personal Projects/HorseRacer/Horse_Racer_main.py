...
### MAIN GAME SCREEN ###
    ...
    def update_main_game(self, round_count):
        if self.round_count != 2 and self.round_count != 8  and self.round_count != 10 and all(fence.rect.x <= -400 for fence in self.fence) and all(bat.rect.x <= -400 for bat in self.bat):
            self.round_count += 1
            for fence in self.fence:
                fence.fence_appear = True
            for bat in self.bat:
                bat.bat_appear = True
        elif self.round_count != 2 and self.round_count != 8 and self.round_count != 10:
            for fence in self.fence:
                fence.fence_appear = False
            for bat in self.bat:
                bat.bat_appear = False
        elif self.round_count == 10 and all(waterdrop.rect.y >= 800 for waterdrop in self.waterdrop):
